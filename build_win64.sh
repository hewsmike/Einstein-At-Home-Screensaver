...
set_mingw() {
    ...
    export CPPFLAGS="-DGLEW_STATIC $CPPFLAGS"
	}
...
build_framework_mingw() {
    ...
    cp -f $ROOT/src/framework/Makefile.win64 Makefile >> $LOGFILE 2>&1 || failure
...
build_ogl_utility_mingw() {
    ...
    cp -f $ROOT/src/ogl_utility/Makefile.win64 Makefile >> $LOGFILE 2>&1 || failure
...
build_product_mingw() {
    ...
    cp $ROOT/../src/$PRODUCT/Makefile.win64 Makefile
    make $makemode PRODUCT=$PRODUCT_NAME >> $LOGFILE 2>&1 || failure
    make install >> $LOGFILE 2>&1 || failure
