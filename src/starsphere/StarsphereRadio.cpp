...
void StarsphereRadio::render(const double timeOfDay) {
...
void StarsphereRadio::prepareSearchInformation() {
    ...
    if(!(m_text_surface = TTF_RenderText_Blended(m_FontText, m_WUSkyPosRightAscension.c_str(), color))){
        ErrorHandler::record("StarsphereRadio::prepareSearchInformation() : can't make SDL_Surface for right asccension!", ErrorHandler::FATAL);
        }
    ...
    if(!(m_text_surface = TTF_RenderText_Blended(m_FontText, m_WUSkyPosDeclination.c_str(), color))){
        ErrorHandler::record("StarsphereRadio::prepareSearchInformation() : can't make SDL_Surface for declination!", ErrorHandler::FATAL);
        }
    ...
    if(!(m_text_surface = TTF_RenderText_Blended(m_FontText, m_WUPercentDone.c_str(), color))){
        ErrorHandler::record("StarsphereRadio::prepareSearchInformation() : can't make SDL_Surface for percent done!", ErrorHandler::FATAL);
        }
    ...
    if(!(m_text_surface = TTF_RenderText_Blended(m_FontText, m_WUCPUTime.c_str(), color))){
        ErrorHandler::record("StarsphereRadio::prepareSearchInformation() : can't make SDL_Surface for CPU time!", ErrorHandler::FATAL);
        }
...
void StarsphereRadio::prepareLogo() {
    ...
    const GLuint TEXTURE_WIDTH = 538;
    const GLuint TEXTURE_HEIGHT = 408;
