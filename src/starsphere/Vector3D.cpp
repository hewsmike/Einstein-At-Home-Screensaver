...
const float Vector3D::NULL_LENGTH(0.0f);
const float Vector3D::NULL_X(0.0f);
const float Vector3D::NULL_Y(0.0f);
const float Vector3D::NULL_Z(0.0f);
...
bool Vector3D::isEqual(const Vector3D& other) const {
   // Assume falsity.
   bool ret_val = false;
   ...
   if((this->x() == other.x()) &&
      (this->y() == other.x()) &&
      (this->z() == other.z())) {
         // Equality of all components gets you the truth!
         ret_val = true;
      }
