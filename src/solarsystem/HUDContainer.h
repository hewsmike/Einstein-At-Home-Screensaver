 ...
 *
 * The key design issue here is that a HUDContainer may be
 * one/both of container and contained.
 *
 * \author Mike Hewson\n
 */
...
class HUDContainer : public HUDItem {
   ...
   protected:
       ...
       *
       * If this does not create a FATAL error when executing, then on
       * completion :
       ...
       *
       * As this depends upon the container's internal layout,
       * then a subclass must enact this.
       *
      ...
      /**
       * \brief Determine and set the base positions of each item. Pure
       *        virtual as only a subclass will know how to do this.
       */
