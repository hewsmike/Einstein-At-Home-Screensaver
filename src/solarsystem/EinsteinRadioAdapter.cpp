        ...
        if(m_WUTemplatePowerSpectrumString.length() == POWERSPECTRUM_BIN_BYTES) {
            ...
            for(int i = 0, j = 0; i < POWERSPECTRUM_BIN_BYTES; i += 2, ++j) {
                try {
                    ...
                    ErrorHandler::record("EinsteinRadioAdapter::parseApplicationInformation() : Error processing power spectrum shared memory data!",
                                         ErrorHandler::WARN);
                    break;
                    }
                }
            ...
            ErrorHandler::record("EinsteinRadioAdapter::parseApplicationInformation() : Invalid power spectrum shared memory data encountered!",
                                 ErrorHandler::WARN);
            }
        }
    }
...
void EinsteinRadioAdapter::processXmlNode(const xmlTextReaderPtr xmlReader,
    ...
    if (nodeValue == NULL) {
        ErrorHandler::record("EinsteinRadioAdapter::processXmlNode() : Error parsing XML node (invalid value)",
                             ErrorHandler::WARN);
        return;
        }
    ...
    try {
        ...
        ErrorHandler::record("EinsteinRadioAdapter::processXmlNode() : Error converting XML reader node content!",
                             ErrorHandler::WARN);
        }
    }
