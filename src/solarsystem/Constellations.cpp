...
Constellations::Constellations(vec_t rad) : radius(rad) {
    ...
    Constellation eridanus("Eridanus");
    eridanus.addToDescription("The river");
    eridanus.add_star(OrdStar(24.42f, -57.24f, 0.45f, OrdStar::SPEC_B, "Achernar"));
    eridanus.add_star(OrdStar(76.96f, -5.09f, 2.78f, OrdStar::SPEC_A, "Cursa"));
    eridanus.add_star(OrdStar(44.56f, -40.31f, 2.88f, OrdStar::SPEC_A, "Acamar"));
    eridanus.add_star(OrdStar(59.50f, -13.51f, 2.97f, OrdStar::SPEC_M, "Zaurak"));
    eridanus.add_star(OrdStar(55.81f, -9.77f, 3.52f, OrdStar::SPEC_K, "Rana"));
    eridanus.add_star(OrdStar(64.47f, -33.80f, 3.55f, OrdStar::SPEC_B, ""));
    eridanus.add_star(OrdStar(34.13f, -51.52f, 3.56f, OrdStar::SPEC_B, ""));
    eridanus.add_star(OrdStar(28.99f, -51.61f, 3.69f, OrdStar::SPEC_G, ""));
    eridanus.add_star(OrdStar(49.88f, -21.78f, 3.70f, OrdStar::SPEC_M, ""));
    eridanus.add_star(OrdStar(53.22f, -9.46f, 3.72f, OrdStar::SPEC_K, "Sadira"));
    eridanus.add_star(OrdStar(68.89f, -30.56f, 3.81f, OrdStar::SPEC_G, "Beemin"));
    eridanus.add_star(OrdStar(44.10f, -8.90f, 3.89f, OrdStar::SPEC_K, "Azha"));
    eridanus.add_star(OrdStar(69.08f, -3.35f, 3.93f, OrdStar::SPEC_B, ""));
    eridanus.add_star(OrdStar(66.01f, -34.02f, 3.97f, OrdStar::SPEC_K, ""));
    eridanus.add_star(OrdStar(71.37f, -3.25f, 4.01f, OrdStar::SPEC_B, ""));
    eridanus.add_star(OrdStar(62.96f, -6.83f, 4.04f, OrdStar::SPEC_F, "Beid"));
    eridanus.add_star(OrdStar(45.60f, -23.62f, 4.08f, OrdStar::SPEC_A, ""));
    eridanus.add_star(OrdStar(40.16f, -39.85f, 4.11f, OrdStar::SPEC_K, ""));
    eridanus.add_star(OrdStar(57.36f, -36.20f, 4.17f, OrdStar::SPEC_G, ""));
    eridanus.add_star(OrdStar(56.70f, -23.25f, 4.22f, OrdStar::SPEC_F, ""));
    eridanus.add_star(OrdStar(36.75f, -47.70f, 4.24f, OrdStar::SPEC_B, ""));
    eridanus.add_star(OrdStar(49.97f, -43.07f, 4.26f, OrdStar::SPEC_G, ""));
    eridanus.add_star(OrdStar(53.45f, -21.02f, 4.26f, OrdStar::SPEC_B, ""));
    eridanus.add_star(OrdStar(41.27f, -18.57f, 4.47f, OrdStar::SPEC_F, ""));
    eridanus.add_link(0, 7);
    eridanus.add_link(1, 14);
    eridanus.add_link(2, 17);
    eridanus.add_link(2, 21);
    eridanus.add_link(3, 4);
    eridanus.add_link(3, 15);
    eridanus.add_link(4, 9);
    eridanus.add_link(5, 13);
    eridanus.add_link(5, 18);
    eridanus.add_link(6, 7);
    eridanus.add_link(6, 20);
    eridanus.add_link(8, 16);
    eridanus.add_link(8, 22);
    eridanus.add_link(9, 11);
    eridanus.add_link(10, 13);
    eridanus.add_link(10, 19);
    eridanus.add_link(11, 23);
    eridanus.add_link(12, 14);
    eridanus.add_link(12, 15);
    eridanus.add_link(16, 23);
    eridanus.add_link(17, 20);
    eridanus.add_link(18, 21);
    eridanus.add_link(19, 22);
    cons_list.push_back(eridanus);
    tot_stars += eridanus.total_stars();
    tot_links += eridanus.total_links();
    ...
    vulpecula.add_star(OrdStar(303.94f, 27.81f, 4.50f, OrdStar::SPEC_K, ""));
    vulpecula.add_star(OrdStar(313.03f, 27.10f, 4.56f, OrdStar::SPEC_G, ""));
...
LookOut Constellations::getView(unsigned int sequence) const {
    ...
    if(cons_list.size() > 0) {
        // Check out of range, return the last Lookout if so.
        if(sequence > (cons_list.size() - 1)) {
            sequence = cons_list.size() - 1;
            }

        // Obtain the current constellation.
        Constellation current_con = cons_list.at(sequence);

        // Get that constellation's centroid sky angular coordinates .
        std::pair<float, float> cons_centroid = current_con.centre();

        // Turn that into a vector from the origin using spherical polars.
        VectorSP centroid = VectorSP(cons_centroid.first, cons_centroid.second, radius);

        // Make another vector set back from the centroid along the radial
        // from the origin.
        Vector3D viewpoint = centroid - centroid.unit() * VIEW_OFFSET;

        // Sit at the viewpoint,
        ret_val.setPosition(viewpoint);
        // looking at the centroid,
        ret_val.setFocus(centroid);
        // with the given view orientation.
        ret_val.setOrientation(VIEW_UP);

        // Include the constellation's name.
        ret_val.addToDescription(current_con.name());

        // Include the constellation's descriptive strings.
...
void Constellations::render(void) {
    ...
    if(current_cycle_state != ALL_OFF) {
        ...
        if((current_cycle_state == STARS_N_LINKS) ||
           (current_cycle_state == ALL_ON)) {
            // The GL_ELEMENT_ARRAY_BUFFER target will be serviced by the
            // other buffer.
      ...
      // Provided we show the names.
        if((current_cycle_state == STARS_N_NAMES) ||
           (current_cycle_state == ALL_ON)) {
...
void Constellations::loadVertexBuffer(void) {
    ...
    // Obtain a heap allocation.
    Vert* buffer_base_ptr = new Vert[tot_stars];
    // Use this pointer to proceed through the map.
    ...
    for(std::vector<Constellation>::const_iterator cs = cons_list.begin();
        ...
        for(std::vector<OrdStar>::const_iterator st = star_list.begin();
            st != star_list.end();
            ++st) {
            // Take the star's spectral type and convert that to RGB color
            // values.
    ...
    // Move the information on the heap into the server side buffer.
...
void Constellations::loadIndexBuffer(void) {
    ...
    // Need to keep track of the indices with respect to the entire set of
    // constellations.
    ...
    for(std::vector<Constellation>::const_iterator cs = cons_list.begin();
        ...
        // Obtain the number of stars in the current constellation.
        ...
        if(total_stars_this_con != 0) {
            ...
            ++lk) {
                ...
                // That ensures that the indices entered here refer to the
                // correct stars in the vertex listing. Need to check that any
                // index within a constellation does not go out of bounds with
                // respect to the number of stars in said constellation. As the
                // link indices are unsigned then only need to check upper index
                // bound.
                if((lk->first < total_stars_this_con) &&
                   (lk->first < total_stars_this_con)) {
    ...
    // Move the information on the heap into the server side buffer.
    buff_obj_indices.loadBuffer(GL_ELEMENT_ARRAY_BUFFER, GL_STATIC_DRAW,
...
void Constellations::createMarkerLists(void) {
    ...
    for(std::vector<Constellation>::iterator cs = cons_list.begin();
        ...
        glNewList(transform_ID, GL_COMPILE);
            ...
            glPushMatrix();
                // Transforms etc ... in reverse order if thinking of
                // "world space" description.

                // Rotate to desired right ascension.
                // Rotate anti-clockwise, seen looking from +ve z-axis toward
                // origin.
                glRotatef(con_centre.first, 0, 0, 1);

                // Rotate to desired declination.
                // Rotate anti-clockwise, seen looking from -ve y-axis toward
                // origin.
                ...
                // Rotate 90 degrees clockwise, seen looking from +ve y-axis
                // toward origin.
                glRotatef(-90, 0, 1, 0);

                // Rotate 90 degrees clockwise, seen looking from +ve z-axis
                // toward origin.
                glRotatef(-90, 0, 0, 1);
