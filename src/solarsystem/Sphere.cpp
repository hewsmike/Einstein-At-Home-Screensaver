...
// an equatorial equilateral triangle ( one point of which is
// at the prime meridian ). This implementation REQUIRES the
// following constants as is.
...
void Sphere::makeVertices(void) {
    ...
    // By what amount in degrees ( if any ) do we stagger alternate
    ...
    for(GLuint stack = 0; stack < num_stacks; ++stack) {
        ...
        // How many vertices per stack, accounting for any stitch vertex ?
        ...
        for(GLuint slice = 0; slice < stack_vertex_count; ++slice) {
            ...
            Vertex current = Vertex(position, normal,
                                    std::pair<GLfloat,GLfloat>(long_text_coord, lat_text_coord));
