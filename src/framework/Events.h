...
class Events {
	public:
		...
		/**
       * \brief Retrieve ( maybe create ) the singleton, possibly initialising
		 *	event management.
       *
       * \param render_interval milliseconds between render calls,
       *                        only relevant for the first call to
       *                        this routine.
       */
   ...
   private:
       ...
       *
       * \param render_interval pointer to an Event which may be filled with
       *                        the detail of said event.
       */
       ...
       * \param character - uppercase printable ISO 8859-1 character
		 *							 or a special key identifier per GLFW.
       * \param action - one of GLFW_PRESS or GLFW_RELEASE.
       */
