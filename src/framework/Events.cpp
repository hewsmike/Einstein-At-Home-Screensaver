...
void Events::init(GLuint render_interval) {
    ...
    // These GLFW callbacks below herewith defined are to 'ordinary'
    // functions ie. not part of any C++ class definition - not even
    // static. The reason is that GLFW is a C library, hence has no
...
int GLFWCALL Events::quit(void) {
    ...
    // Allow the window closure to continue
    // ( GL_FALSE here would prevent that )
    return GL_TRUE;
    }
